...
function profileAdjustment(profile: string) {
  ...
  if (["premium", "high", "high_value"].includes(normalized)) {
    return 5;
  }
  if (["established", "mid_high"].includes(normalized)) {
    return 3;
  }
  if (["industrial", "leisure", "innovation"].includes(normalized)) {
    return -2;
  }
  return 0;
}
...
function calculateDecision(
  ...
  return {
    ...
    predicted_price_per_sqm: Number(predictions.predicted_price_per_sqm.toFixed(2)),
    predicted_estimated_value_aed: Number(
    ...
    reason: `${recommendation}: score ${score.toFixed(1)}/100 with ${marginPct.toFixed(
      1,
    )}% margin, ${districtYield.toFixed(1)}% district yield, ${potential.toFixed(
      1,
    ...
    warnings: [
      ...
      "ONNX browser bundle uses XGBoost-compatible models for portability.",
    ],
    ...
    current_status:
      typeof parcel.current_status === "string" ? parcel.current_status : undefined,
    ...
    data_source: "ONNX browser bundle generated from starter-kit CSVs",
  };
}
  ...
  const sessions = Object.fromEntries(
    await Promise.all(
      MODEL_KEYS.map(async (key) => {
        ...
        return [key, await ort.InferenceSession.create(modelUrl(modelSchema.onnx_path))];
      }),
    ),
  ) as Record<ModelKey, ort.InferenceSession>;
